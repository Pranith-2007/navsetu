...
function robotRow(r){return `<div class="robot-row"><div class="robot-name"><span class="mini-bot" style="--rc:${r.color}">${r.id}</span><div><b>${r.status}</b><small>${esc(r.task||'Live route telemetry')}</small></div></div><div class="battery"><span>${Math.round(r.battery)}%</span><i><em style="width:${r.battery}%"></em></i></div><div class="speed">${r.speed.toFixed(1)}<small> m/s</small></div><button onclick="selectRobot('${r.id}')">→</button></div>`}
